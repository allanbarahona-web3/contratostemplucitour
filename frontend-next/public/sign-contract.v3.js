...
const showError = (msg) => {
  ...
  showEl(errorStateEl);
  const message = String(msg || "Ocurrio un error.").trim();
  const normalized = message.toLowerCase();
  const isContractClosed =
    normalized.includes("cerrado o firmado") ||
    normalized.includes("ya fue firmado") ||
    normalized.includes("proceso cerrado");

  if (errorTitleEl) {
    errorTitleEl.textContent = isContractClosed
      ? "Proceso de firma cerrado"
      : "No fue posible abrir este enlace";
  }
  if (errorMessageEl) errorMessageEl.textContent = message || "Ocurrio un error.";
  if (errorHintEl) {
    errorHintEl.textContent = isContractClosed
      ? "Este enlace ya no esta disponible. El contrato final fue completado y enviado en PDF firmado."
      : "Si necesitas ayuda, contacta a tu asesor de viajes.";
  }
  hideEl(readStepEl);
  hideEl(signStepEl);
  hideEl(successStepEl);
